...
#-*- coding: utf-8 -*-
# ******************************************************************************
# 程序名称:     ydz_app_statics_uin_pageview_nps.py
# 功能描述:     油点赞数据统计——页面访问NPS数据统计
# 输入参数:     yyyymmdd    例如：20140113
# 目标表名:     ieg_qt_community_app.tb_ydz_app_page_view_nps
...
##    res = tdw.execute(sql)
    ...
    sql = """
    INSERT OVERWRITE TABLE tb_ydz_app_pageview_avg_temp_%s
    SELECT
    appid,
    machineflag,
    moduleid,
    ...
    CASE WHEN seg_9_account = 0 THEN 10000000000.0 ELSE seg_9_pv/seg_9_account END AS seg_9_pv,
    CASE WHEN seg_9_account = 0 THEN 10000000000.0 ELSE seg_9_time/seg_9_account END AS seg_9_time
    FROM 
    (
    SELECT
    appid,
    ...
    FROM  tb_ydz_user_score_data PARTITION (p_%s) tmp 
     ...
     JOIN 
    tb_ydz_mta_pageview_action_accumulate  PARTITION (p_%s) b
    ON (a.appid = b.appid AND a.account = b.accountid AND a.machinetype = b.machineflag )
    )t
    GROUP BY appid,    machineflag,    moduleid,    kv
    )t1
     """%(sDate,sDate,sDate)
    tdw.WriteLog(sql)
    res = tdw.execute(sql)
    ...
    ##NPS值结果表
    sql = """
    CREATE TABLE IF NOT EXISTS tb_ydz_app_page_view_nps
    (
    ...
    sql = """
    DELETE FROM tb_ydz_app_page_view_nps WHERE dtstatdate = %s
     """%(sDate)
    tdw.WriteLog(sql)
    res = tdw.execute(sql)
    ...
    ##计算NPS结果值
    sql = """
    INSERT TABLE tb_ydz_app_page_view_nps
    SELECT
    %s  AS dtstatdate,
    appid,
    machineflag,
    ...
    FROM tb_ydz_app_pageview_avg_temp_%s a 
    JOIN
      tb_ydz_mta_pageview_action_accumulate  PARTITION (p_%s) b
    ON(a.appid = b.appid AND a.moduleid = b.moduleid AND a.machineflag = b.machineflag AND a.kv = b.kv)
    )t
    GROUP BY appid,moduleid, machineflag,kv
     """%(sDate,sDate,sDate)
    tdw.WriteLog(sql)
    res = tdw.execute(sql)
    
    
    sql = """ DROP TABLE tb_ydz_app_pageview_avg_temp_%s"""%(sDate)
    tdw.WriteLog(sql)
    res = tdw.execute(sql)
