...
#-*- coding: utf-8 -*-
# ******************************************************************************
# 程序名称:     lol_app_dau_tb_app_effect_dnu_data.py
# 功能描述:     掌盟掌火有效新进用户统计
# 输入参数:     yyyymmdd    例如：20160309
# 目标表名:     ieg_qt_community_app.tb_app_effect_dnu_data
...
def TDW_PL(tdw, argv=[]):
    ...
    sql = """
            CREATE TABLE IF NOT EXISTS tb_app_effect_dnu_data
            (
            dtstatdate INT COMMENT '统计日期',
            iclienttype INT COMMENT '客户端类型',
            idaysflag INT COMMENT '相隔时间',
            idunuin BIGINT COMMENT '总新进用户',
            ieffectuin BIGINT COMMENT '有效新进用户'
            ) """
    tdw.WriteLog(sql)
    res = tdw.execute(sql)

    sql="""DELETE FROM tb_app_effect_dnu_data WHERE dtstatdate = %s""" % (sDate)
    tdw.WriteLog(sql)
    res = tdw.execute(sql)
   ...
   ##月有效新进用户
    sql = """
            INSERT TABLE tb_app_effect_dnu_data
            SELECT
            %(sDate)s AS dtstatdate,
            iclienttype,
            30 AS idaysflag,
            COUNT(DISTINCT iuin) AS iduncnt,
            ...
            FROM tb_app_reg_account PARTITION (p_%(sDate)s) a WHERE dtstatdate = %(sDate)s AND iregdate > %(pre_30_date_str)s AND iregdate <= %(sDate)s
            )t
            )t1
    ...
    sql = """
            ...
            FROM tb_app_reg_account PARTITION (p_%(sDate)s) a WHERE dtstatdate = %(sDate)s AND iregdate > %(pre_7_date_str)s AND iregdate <= %(sDate)s
            )t
            )t1
            GROUP BY iclienttype
    ...
    sql = """
            ...
            FROM tb_app_reg_account PARTITION (p_%(sDate)s) a WHERE dtstatdate = %(sDate)s AND iregdate > %(pre_14_date_str)s AND iregdate <= %(sDate)s
            )t
            )t1
            GROUP BY iclienttype
