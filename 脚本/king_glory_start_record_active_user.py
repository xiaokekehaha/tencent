...
#-*- coding: utf-8 -*-
# ******************************************************************************
# 程序名称:     king_glory_start_record_active_user.py
# 功能描述:     掌萌掌火观看视频时长统计
# 输入参数:     yyyymmdd    例如：20140113
# 目标表名:     ieg_qt_community_app::king_glory_start_record_active_user
...
#创建结果表   
    tdw.WriteLog("== create table king_glory_start_record_active_user==")
    sql = """CREATE TABLE IF NOT EXISTS king_glory_start_record_active_user(
        sdate bigint,
        game_id string,
        version string,
        platform string,
        system string,
        smachinetype string,
        start_record_usernum string,
        start_record_times bigint  
        )"""
    tdw.WriteLog(sql)    
    res = tdw.execute(sql)
    tdw.WriteLog("== create table king_glory_start_record_active_user success==")

#确认插入数据日期的唯一性    
    tdw.WriteLog("== check date==")
    sql="""delete from king_glory_start_record_active_user where sdate=%s"""%(sDate)
    res = tdw.execute(sql)
    tdw.WriteLog("== check date success==")
    
#插入数据        
    tdw.WriteLog("== insert table==")
    sql="""INSERT TABLE  king_glory_start_record_active_user 
    SELECT 
    %s,
    ...
    ELSE t.smachinetype
    END AS smachinetype,
    COUNT(DISTINCT(t.suuid)) as start_record_usernum,
    COUNT(*) as  start_record_times
    FROM 
        (SELECT 
        sgameid,   
        sversion,
    ...
    WHERE t.platform != 'other' AND t.system != 'other'
