...
#-*- coding: utf-8 -*-
# ******************************************************************************
# 程序名称:     king_glory_record_user_days4_remain.py
# 功能描述:     掌萌掌火观看视频时长统计
# 输入参数:     yyyymmdd    例如：20140113
# 目标表名:     ieg_qt_community_app::king_glory_record_user_days4_remain.py
...
#创建结果表   
    tdw.WriteLog("== create table king_glory_record_user_days4_remain==")
    sql = """CREATE TABLE IF NOT EXISTS king_glory_record_user_days4_remain(
        sdate bigint,
        game_id string,
        version string,
        platform string,
    ...
    tdw.WriteLog(sql)    
    res = tdw.execute(sql)
    tdw.WriteLog("== create table king_glory_record_user_days4_remain success==")

#确认插入数据日期的唯一性    
    tdw.WriteLog("== check date==")
    sql="""delete from king_glory_record_user_days4_remain where sdate=%s"""%(sDate)
    res = tdw.execute(sql)
    tdw.WriteLog("== check date success==")

#插入数据        
    tdw.WriteLog("== insert table==")
    sql="""INSERT TABLE  king_glory_record_user_days4_remain 
    SELECT 
    %s,
    t1.sgameid,
    t1.sversion,
    ...
    """% (sDate,sDate,sDate,day4_str,day4_str,day4_str,day4_str)  
    tdw.WriteLog(sql)
    res = tdw.execute(sql)
