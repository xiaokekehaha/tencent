...
#-*- coding: utf-8 -*-
# ******************************************************************************
# 程序名称:     herotime_tgp_statistic.py
# 功能描述:     手游宝全民突击数据
# 输入参数:     yyyymmdd    例如：20160120
# 目标表名:     ieg_qt_community_app::herotime_tgp_statistic
...
def TDW_PL(tdw, argv=[]):
    ...
    sql = """
            CREATE TABLE IF NOT EXISTS herotime_tgp_statistic
            (
            fdate int,
            event  string,
            pv  int,
    ...
    sql="""delete from herotime_tgp_statistic where fdate=%s""" %(sDate)
    tdw.WriteLog(sql)
    res = tdw.execute(sql)
...
#TGP核心数据计算
    ...
    sql = """
    insert table herotime_tgp_statistic
    SELECT 
    t.f_date,
    t.event,
    ...
    f_pvid,
    CASE 
    WHEN f_url = '/php_cgi/tgp_hero/html/index.html' THEN 'home_page'
    WHEN f_url = '/php_cgi/tgp_hero/html/play_video.html' THEN 'play_page'
    WHEN f_url = '/php_cgi/tgp_hero/html/video_rank.html' THEN 'more_page'
    ELSE 'other'
    END AS event
    FROM teg_dw_tcss::tcss_qt_qq_com
    WHERE f_dm = 'QT.QQ.COM' AND f_date=%s)t 
    WHERE event != 'other'
