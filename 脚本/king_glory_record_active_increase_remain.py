...
#-*- coding: utf-8 -*-
# ******************************************************************************
# 程序名称:     king_glory_record_active_increase_remain.py
# 功能描述:     掌萌掌火观看视频时长统计
# 输入参数:     yyyymmdd    例如：20140113
# 目标表名:     ieg_qt_community_app::king_glory_record_active_increase_remain
...
#创建结果表   
    tdw.WriteLog("== create table king_glory_record_active_increase_remain==")
    sql = """CREATE TABLE IF NOT EXISTS king_glory_record_active_increase_remain(
        sdate bigint,
        cdate bigint,
        game_id string,
        type string,
    ...
    tdw.WriteLog(sql)    
    res = tdw.execute(sql)
    tdw.WriteLog("== create table king_glory_record_active_increase_remain success==")

#确认插入数据日期的唯一性    
    tdw.WriteLog("== check date==")
    sql="""delete from king_glory_record_active_increase_remain where sdate=%s"""%(sDate)
    res = tdw.execute(sql)
    tdw.WriteLog("== check date success==")
    
#插入数据        
    tdw.WriteLog("== insert table==")
    sql="""INSERT TABLE  king_glory_record_active_increase_remain 
    SELECT 
    %s,
    %s,
    t1.game_id,
    ...
    """%(day2_str,sDate,day2_str,day2_str,day3_str,day2_str,day2_str,day3_str,sDate,sDate) 
    tdw.WriteLog(sql)
    res = tdw.execute(sql)
    ...
    FROM 
        ...
        GROUP BY r1.sgameid)t2 
        ON t1.game_id=t2.sgameid
    """%(day3_str,sDate,day3_str,day3_str,day4_str,day3_str,day3_str,day4_str,sDate,sDate) 
    tdw.WriteLog(sql)
    res = tdw.execute(sql)
    
    tdw.WriteLog("== day4_remain insert table==")
    sql="""INSERT TABLE  king_glory_record_active_increase_remain 
    SELECT 
    %s,
    %s,
    ...
    FROM 
        ...
        GROUP BY r1.sgameid)t2 
        ON t1.game_id=t2.sgameid
    """%(day4_str,sDate,day4_str,day4_str,day5_str,day4_str,day4_str,day4_str,sDate,sDate) 
    tdw.WriteLog(sql)
    res = tdw.execute(sql)
    ...
    FROM 
        ...
        GROUP BY r1.sgameid)t2 
        ON t1.game_id=t2.sgameid
    """%(day5_str,sDate,day5_str,day5_str,day6_str,day5_str,day5_str,day6_str,sDate,sDate) 
    tdw.WriteLog(sql)
    res = tdw.execute(sql)
    ...
    FROM 
        ...
        GROUP BY r1.sgameid)t2 
        ON t1.game_id=t2.sgameid
    """%(day6_str,sDate,day6_str,day6_str,day7_str,day6_str,day6_str,day7_str,sDate,sDate) 
    tdw.WriteLog(sql)
    res = tdw.execute(sql)
    
    tdw.WriteLog("==day7_remain insert table==")
    sql="""INSERT TABLE  king_glory_record_active_increase_remain 
    SELECT 
    %s,
    %s,
    ...
    FROM 
        ...
        GROUP BY r1.sgameid)t2 
        ON t1.game_id=t2.sgameid
    """%(day7_str,sDate,day7_str,day7_str,day8_str,day7_str,day7_str,day8_str,sDate,sDate) 
    tdw.WriteLog(sql)
    res = tdw.execute(sql)
