...
def TDW_PL(tdw, argv=[]):
    ...
    ##统计停留时长
    sql = """
            CREATE TABLE IF NOT EXISTS syb_bbs_source_data_v2
            (
            fdate int,
            plat string,
            src string,
    ...
    sql="""delete from syb_bbs_source_data_v2 where fdate=%s  """ % (sDate)
    res = tdw.execute(sql)

    ##全体数据不分平台部分来源
    sql = """
    insert  table  syb_bbs_source_data_v2
    select 
    %s,
    'all',
    'all',
    ...
    CASE WHEN GROUPING(fi) = 1 THEN -100 ELSE fi  END AS  block
