...
def TDW_PL(tdw, argv=[]):
    ...
    pre_date = today_date - datetime.timedelta(days = 3*int(period_days))
    pre_date_str = pre_date.strftime("%Y%m%d") 
    ...
    ##----创建结果表
    sql = """
            CREATE TABLE IF NOT EXISTS tb_gpcd_reback_user
            (
            dtstatdate INT COMMENT '统计日期',
            business STRING COMMENT '业务类型',
            period int COMMENT '周期天数',
            active_num BIGINT COMMENT '第一个周期活跃用户数',
            reback_num BIGINT COMMENT '当前周期回流用户数'
            )"""
    tdw.WriteLog("%s"%sql)
    res = tdw.execute(sql)

    sql = "delete from tb_gpcd_reback_user where period=%s and dtstatdate=%s"%(period_days,sDate)
    ...
    sql="""
        insert table tb_gpcd_reback_user
        ...
        WHERE  dtstatdate = %s and ilastactdate >= %s  and substr(cbitmap, %d, %d) != "%s" and substr(cbitmap, 1, %d) != "%s" and substr(cbitmap, %d, %d) = "%s"
        group by business 
        ) b
        on (a.business = b.business)
        """ %(sDate,period,sDate,sDate,pre_date_str,2*period + 1,period,zero_bits,sDate,sDate,pre_date_str,2*period + 1,period,zero_bits,period,zero_bits,period + 1, period,zero_bits)
