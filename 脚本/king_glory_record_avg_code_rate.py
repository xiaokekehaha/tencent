...
#-*- coding: utf-8 -*-
# ******************************************************************************
# 程序名称:     king_glory_record_avg_code_rate.py
# 功能描述:     掌萌掌火观看视频时长统计
# 输入参数:     yyyymmdd    例如：20140113
# 目标表名:     ieg_qt_community_app::king_glory_record_avg_code_rate
...
#创建结果表   
    tdw.WriteLog("== create table king_glory_record_avg_code_rate==")
    sql = """CREATE TABLE IF NOT EXISTS king_glory_record_avg_code_rate(
        sdate bigint,
        game_id string,
        version string,
        platform string,
    ...
    tdw.WriteLog(sql)    
    res = tdw.execute(sql)
    tdw.WriteLog("== create table king_glory_record_avg_code_rate success==")

#确认插入数据日期的唯一性    
    tdw.WriteLog("== check date==")
    sql="""delete from king_glory_record_avg_code_rate where sdate=%s"""%(sDate)
    res = tdw.execute(sql)
    tdw.WriteLog("== check date success==")
    
#插入数据        
    tdw.WriteLog("== insert table==")
    sql="""INSERT TABLE  king_glory_record_avg_code_rate 
    SELECT 
    %s,
    ...
    ELSE t.smachinetype
    END AS smachinetype,
    avg(t.sreserve1) as avg_code_rate
    FROM 
        (SELECT 
        sgameid,
        sversion,
