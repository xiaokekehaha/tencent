...
#-*- coding: utf-8 -*-
# ******************************************************************************
# 程序名称:     ydz_app_recommend_sys_user_food_table_inform_env.py
# 功能描述:     游点赞推荐系统数据统计——用户使用行为表,测试环境
# 输入参数:     yyyymmdd    例如：20140113
# 目标表名:     ieg_qt_community_app.tb_ydz_app_recommend_sys_usr_food_informal_env
...
##    res = tdw.execute(sql)
    ...
    sql = """
    CREATE TABLE IF NOT EXISTS tb_ydz_app_recommend_sys_usr_food_informal_env
    (
    ...
    sql = """
    ALTER TABLE tb_ydz_app_recommend_sys_usr_food_informal_env DROP PARTITION (p_%(sDate)s)
     """%({"sDate":sDate})
    tdw.WriteLog(sql)
    res = tdw.execute(sql)
    ...
    sql = """
    ALTER TABLE tb_ydz_app_recommend_sys_usr_food_informal_env ADD PARTITION p_%(sDate)s VALUES IN (%(sDate)s)
     """%({"sDate":sDate})
    tdw.WriteLog(sql)
    res = tdw.execute(sql)
    ...
    ##记录历史信息，这个作为备注不执行
    '''
    sql = """ INSERT TABLE tb_ydz_app_recommend_sys_usr_food_informal_env
    SELECT 
    20160516 AS dtstatdate,
    accountid,
    feed_id,
    ...
    ts,
    du
    FROM teg_mta_intf::ieg_youxishengjing WHERE sdate = 20160516 AND ei IN ( 'feed_read','praise','big_picture','comment_add','comment_view','share') AND get_json_object(kv,'$.server_env') = '1' 
    )t
    ...
    sql = """ DROP TABLE tb_ydz_app_recommend_sys_usr_food_informal_env_temp_%(sDate)s """%({"sDate":sDate})
    tdw.WriteLog(sql)
    res = tdw.execute(sql)
    
    
    sql = """
    CREATE TABLE IF NOT EXISTS tb_ydz_app_recommend_sys_usr_food_informal_env_temp_%(sDate)s
    (
    accountid BIGINT COMMENT '用户账号ID',
    feed_id INT COMMENT '用户操作的feedid',
    read_flag INT COMMENT '用户是否阅读的标记',
    ...
    ###临时数据写入
    sql = """
    INSERT OVERWRITE TABLE tb_ydz_app_recommend_sys_usr_food_informal_env_temp_%(sDate)s
    SELECT 
    accountid,
    feed_id,
    CASE WHEN SUM(read_flag) > 0 THEN 1 ELSE 0 END AS read_flag,
    ...
    ei ,
    ts,
    CASE WHEN get_json_object(kv,'$.duration') IS NOT NULL  THEN CAST (get_json_object(kv,'$.duration') AS BIGINT) ELSE CAST(0 AS BIGINT) END AS du
    FROM teg_mta_intf::ieg_youxishengjing WHERE sdate =  %(sDate)s AND ei IN ( 'feed_read','praise','big_picture','comment_add','comment_view','share') AND get_json_object(kv,'$.server_env') = '1'
    )t
    WHERE accountid IS NOT NULL AND accountid != '0' AND feed_id IS NOT NULL AND feed_id != '0'
    GROUP BY accountid,feed_id,ei
    )a 
    ...
    ##求得最终结果
    sql = """
    INSERT TABLE tb_ydz_app_recommend_sys_usr_food_informal_env 
    ...
    last_aciton_ts
    FROM 
    tb_ydz_app_recommend_sys_usr_food_informal_env_temp_%(sDate)s a
    ...
    feed_id,
    read_flag,
    CASE WHEN feed_read_time < 0 THEN 0 ELSE feed_read_time END AS feed_read_time,
    zan_flag,
    big_picture_flag,
    comment_add_flag,
    comment_view_flag,
    share_flag,
    last_aciton_ts
    FROM tb_ydz_app_recommend_sys_usr_food_informal_env PARTITION (p_%(pre_date_str)s) b
