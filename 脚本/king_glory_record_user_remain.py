...
#-*- coding: utf-8 -*-
# ******************************************************************************
# 程序名称:     king_glory_record_user_remain.py
# 功能描述:     掌萌掌火观看视频时长统计
# 输入参数:     yyyymmdd    例如：20140113
# 目标表名:     ieg_qt_community_app::king_glory_record_user_remain.py
...
#连接tdw， ieg_qt_community_app数据库
    ...
    #sql = """drop table king_glory_record_user_remain"""
    #res = tdw.execute(sql)

#创建结果表   
    tdw.WriteLog("== create table king_glory_record_user_remain==")
    sql = """CREATE TABLE IF NOT EXISTS king_glory_record_user_remain(
        sdate bigint,
        cdate bigint,
        game_id string,
        version string,
    ...
    tdw.WriteLog(sql)    
    res = tdw.execute(sql)
    tdw.WriteLog("== create table king_glory_record_user_remain success==")

#确认插入数据日期的唯一性    
    tdw.WriteLog("== check date==")
    sql="""delete from king_glory_record_user_remain where cdate=%s"""%(sDate)
    res = tdw.execute(sql)
    tdw.WriteLog("== check date success==")

#插入数据        
    tdw.WriteLog("== insert table==")
    sql="""INSERT TABLE  king_glory_record_user_remain 
    SELECT 
    %s,
    %s,
    t1.sgameid,
    ...
    """%(day2_str,sDate,sDate,sDate,day2_str,day2_str,day2_str,day2_str) 
    tdw.WriteLog(sql)
    res = tdw.execute(sql)
    
    tdw.WriteLog("== day3_remain insert table==")
    sql="""INSERT TABLE  king_glory_record_user_remain 
    SELECT 
    %s,
    %s,
    ...
    t1.iAccountType  ,
    'day3_remain',
    t1.days3_user_num/t2.day1_user_num as days3_remain
    FROM 
    (SELECT 
    CASE WHEN grouping(tt1.sgameid) = 1 THEN 'all'
    ELSE tt1.sgameid
    ...
    ELSE tt1.iAccountType
    END as iAccountType,
    COUNT(DISTINCT(tt1.suuid)) as days3_user_num
    FROM 
        (SELECT 
        sgameid,
        sversion,
    ...
    GROUP BY cube(sgameid),cube(sversion),cube(iClientType),cube(iAccountType))t2
    ON t1.sgameid = t2.sgameid AND t1.sversion = t2.sversion AND t1.iClientType = t2.iClientType AND t1.iAccountType = t2.iAccountType
    """%(day3_str,sDate,sDate,sDate,day3_str,day3_str,day3_str,day3_str) 
    tdw.WriteLog(sql)
    res = tdw.execute(sql)
    ...
    t1.iAccountType  ,
    'day4_remain',
    t1.days4_user_num/t2.day1_user_num as days4_remain
    FROM 
    (SELECT 
    CASE WHEN grouping(tt1.sgameid) = 1 THEN 'all'
    ELSE tt1.sgameid
    ...
    ELSE tt1.iAccountType
    END as iAccountType,
    COUNT(DISTINCT(tt1.suuid)) as days4_user_num
    FROM 
        (SELECT 
        sgameid,
        sversion,
    ...
    GROUP BY cube(sgameid),cube(sversion),cube(iClientType),cube(iAccountType))t2
    ON t1.sgameid = t2.sgameid AND t1.sversion = t2.sversion AND t1.iClientType = t2.iClientType AND t1.iAccountType = t2.iAccountType
    """% (day4_str,sDate,sDate,sDate,day4_str,day4_str,day4_str,day4_str)  
    tdw.WriteLog(sql)
    res = tdw.execute(sql)
    
    
    tdw.WriteLog("== day5_remain insert table==")
    sql="""INSERT TABLE  king_glory_record_user_remain 
    SELECT 
    %s,
    %s,
    ...
    t1.iAccountType  ,
    'day5_remain',
    t1.days5_user_num/t2.day1_user_num as days5_remain
    FROM 
    (SELECT 
    CASE WHEN grouping(tt1.sgameid) = 1 THEN 'all'
    ELSE tt1.sgameid
    ...
    ELSE tt1.iAccountType
    END as iAccountType,
    COUNT(DISTINCT(tt1.suuid)) as days5_user_num
    FROM 
        (SELECT 
        sgameid,
        sversion,
    ...
    GROUP BY cube(sgameid),cube(sversion),cube(iClientType),cube(iAccountType))t2
    ON t1.sgameid = t2.sgameid AND t1.sversion = t2.sversion AND t1.iClientType = t2.iClientType AND t1.iAccountType = t2.iAccountType
    """%(day5_str,sDate,sDate,sDate,day5_str,day5_str,day5_str,day5_str)  
    tdw.WriteLog(sql)
    res = tdw.execute(sql)
    
    tdw.WriteLog("== day6_remain insert table==")
    sql="""INSERT TABLE  king_glory_record_user_remain 
    SELECT 
    %s,
    %s,
    ...
    t1.iAccountType  ,
    'day6_remain',
    t1.days6_user_num/t2.day1_user_num as days6_remain
    FROM 
    (SELECT 
    CASE WHEN grouping(tt1.sgameid) = 1 THEN 'all'
    ELSE tt1.sgameid
    ...
    ELSE tt1.iAccountType
    END as iAccountType,
    COUNT(DISTINCT(tt1.suuid)) as days6_user_num
    FROM 
        (SELECT 
        sgameid,
        sversion,
    ...
    GROUP BY cube(sgameid),cube(sversion),cube(iClientType),cube(iAccountType))t2
    ON t1.sgameid = t2.sgameid AND t1.sversion = t2.sversion AND t1.iClientType = t2.iClientType AND t1.iAccountType = t2.iAccountType
    """%(day6_str,sDate,sDate,sDate,day6_str,day6_str,day6_str,day6_str) 
    tdw.WriteLog(sql)
    res = tdw.execute(sql)
    
    tdw.WriteLog("==days7_remain insert table==")
    sql="""INSERT TABLE  king_glory_record_user_remain 
    SELECT 
    %s,
    %s,
    ...
    t1.iAccountType  ,
    'days7_remain',
    t1.days7_user_num/t2.day1_user_num as days7_remain
    FROM 
    (SELECT 
    CASE WHEN grouping(tt1.sgameid) = 1 THEN 'all'
    ELSE tt1.sgameid
    ...
    ELSE tt1.iAccountType
    END as iAccountType,
    COUNT(DISTINCT(tt1.suuid)) as days7_user_num
    FROM 
        (SELECT 
        sgameid,
        sversion,
    ...
    GROUP BY cube(sgameid),cube(sversion),cube(iClientType),cube(iAccountType))t2
    ON t1.sgameid = t2.sgameid AND t1.sversion = t2.sversion AND t1.iClientType = t2.iClientType AND t1.iAccountType = t2.iAccountType
    """%(day7_str,sDate,sDate,sDate,day7_str,day7_str,day7_str,day7_str) 
    tdw.WriteLog(sql)
    res = tdw.execute(sql)
