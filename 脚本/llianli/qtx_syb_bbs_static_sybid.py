...
#-*- coding: utf-8 -*-
# ******************************************************************************
# 程序名称:     qtx_syb_bbs_static_sybid.py
# 功能描述:     手游宝按照手游宝ID统计论坛来源
# 输入参数:     yyyymmdd    例如：20140113
# 目标表名:     ieg_qt_community_app.syb_bbs_source_statics_sybid
...
def TDW_PL(tdw, argv=[]):
    ...
    ##统计停留时长
    sql = """
            CREATE TABLE IF NOT EXISTS syb_bbs_source_statics_sybid
            (
            fdate int,
            plat string,
            src string,
    ...
    sql="""delete from syb_bbs_source_statics_sybid where fdate=%s  """ % (sDate)
    res = tdw.execute(sql)

    ##全体数据不分平台部分来源
    sql = """
    insert  table  syb_bbs_source_statics_sybid
    select 
    %s,
    'all',
    'all',
