...
#-*- coding: utf-8 -*-
# ******************************************************************************
# 程序名称:     ydz_app_staics_uin_pi_nps.py
# 功能描述:     油点赞数据统计——页面ID的统计NPS值
...
##    res = tdw.execute(sql)
    ...
    sql = """
            CREATE TABLE IF NOT EXISTS tb_ydz_app_page_action_avg_temp_%s
            (
            appid BIGINT COMMENT 'APPID',
            pi STRING COMMENT '页面信息',
    ...
    sql = """
    INSERT OVERWRITE TABLE tb_ydz_app_page_action_avg_temp_%s
    SELECT
    appid,
    pi,
    ...
    SUM(time) AS time
    FROM 
     tb_ydz_app_page_action_width_table 
     GROUP BY appid,accountid,ei
     )b 
    ...
    sql = """
    CREATE TABLE IF NOT EXISTS tb_ydz_app_page_nps
    (
    dtstatdate INT COMMENT '统计日期',
    appid INT COMMENT '应用ID',
    pi STRING COMMENT '页面名称',
    ...
    sql = """
    DELETE FROM tb_ydz_app_page_nps WHERE  dtstatdate = %s 
    """%(sDate)
    tdw.WriteLog(sql)
    res = tdw.execute(sql)
    ...
    sql = """
    INSERT TABLE tb_ydz_app_page_nps
    SELECT
    %s AS dtstatdate,
    appid,
    pi,
    ...
    FROM tb_ydz_app_page_action_avg_temp_%s a 
    JOIN
    (
    SELECT
    appid,
    ...
    ON(a.appid = b.appid AND a.pi = b.ei)
    )t
    GROUP BY appid,pi 
    """%(sDate,sDate)
    tdw.WriteLog(sql)
    ...
    sql = """ DROP TABLE tb_ydz_app_page_action_avg_temp_%s """%(sDate)
    tdw.WriteLog(sql)
    res = tdw.execute(sql)
