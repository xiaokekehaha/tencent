...
#玩家上传视频各渠道观看人数/次数　整体
    sql = """delete from player_watch_user_times_20160307_20160313"""
    res = tdw.execute(sql)

    tdw.WriteLog("==player_watch_user_times_20160307_20160313==")
    sql = """INSERT TABLE player_watch_user_times_20160307_20160313
SELECT 
t1.platform,
COUNT(DISTINCT(t1.uin)) as user_num,
COUNT(*) as times ,
SUM(t1.du) as watch_time 
FROM 
(SELECT 
CASE 
    WHEN (platform = 'lol' or platform = 'cf' ) THEN split(split(split(vid,'/')[4],'_')[1],'\\.')[0] 
    ELSE vid
    END AS vid,
platform ,
uin,
...
#玩家视频观看次数区间分布
    sql = """delete from player_vid_watch_times_20160307_20160313"""
    res = tdw.execute(sql)
    
    tdw.WriteLog("==player_vid_watch_times_20160307_20160313==")  
    sql = """INSERT TABLE player_vid_watch_times_20160307_20160313
SELECT 
t1.vid,
...
GROUP BY t1.vid """
    ...
    sql = """delete from  player_watch_times_vid"""
    res = tdw.execute(sql)
    
    sql = """INSERT TABLE player_watch_times_vid
SELECT 
t1.watch_times_zone,
...
FROM player_vid_watch_user_times)t1"""
    tdw.WriteLog(sql)
    res = tdw.execute(sql)
    
#玩家上传视频时长分布
    sql = """delete from player_watch_time_zone_20160307_20160313"""
    res = tdw.execute(sql)
    
    tdw.WriteLog("==player_vid_watch_times_20160307_20160313==")  
    sql = """INSERT TABLE player_watch_time_zone_20160307_20160313
SELECT 
t1.platform,
t1.watch_time_zone,
COUNT(DISTINCT(t1.uin)) as user_num,
