...
#-*- coding: utf-8 -*-
# ******************************************************************************
# 程序名称:     king_glory_success_generate_vedio_active_user.py
# 功能描述:     掌萌掌火观看视频时长统计
# 输入参数:     yyyymmdd    例如：20140113
# 目标表名:     ieg_qt_community_app::king_glory_success_generate_vedio_active_user
...
#创建结果表   
    tdw.WriteLog("== create table king_glory_success_generate_vedio_active_user==")
    sql = """CREATE TABLE IF NOT EXISTS king_glory_success_generate_vedio_active_user(
        sdate bigint,
        game_id string,
        version string,
        platform string,
        system string,
        smachinetype string,
        success_generate_usernum bigint  ,
        success_generate_times bigint
        )"""
    tdw.WriteLog(sql)    
    res = tdw.execute(sql)
    tdw.WriteLog("== create table king_glory_success_generate_vedio_active_user success==")

#确认插入数据日期的唯一性    
    tdw.WriteLog("== check date==")
    sql="""delete from king_glory_success_generate_vedio_active_user where sdate=%s"""%(sDate)
    res = tdw.execute(sql)
    tdw.WriteLog("== check date success==")
    
#插入数据        
    tdw.WriteLog("== insert table==")
    sql="""INSERT TABLE  king_glory_success_generate_vedio_active_user 
    SELECT 
    %s,
    ...
    FROM 
        ...
        WHERE saction='video_generate' AND ssubaction='success_generate' AND (tdbank_imp_date BETWEEN '%s00' AND '%s24') )t 
